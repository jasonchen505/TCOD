# OPD Multi-Turn (Paper Implementation)

Official codebase for our paper on multi-turn on-policy distillation with TCOD training variants.

This repository currently focuses on **implemented and runnable** workflows only:

- `OPD` (multi-turn on-policy distillation)
- `TCOD-b2f` (Backward-to-Forward)
- `TCOD-f2b` (Forward-to-Backward)
- Environments: `ALFWorld`, `WebShop`, `ScienceWorld`
...
Please replace this section with your paper citation:
...
```bash
conda create -n trinity1 python=3.12
conda activate trinity1

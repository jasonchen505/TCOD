...
class JSDAdvantage(AdvantageFn):
    ...
    def __call__(self, exps: DataProto, **kwargs) -> Tuple[DataProto, Dict]:
        ...
        advantages = kl_coef * JSD (same sign convention as OnPolicyDistillAdvantage).
        When lambda_coef=1: JSD = KL(teacher||student) = (teacher - student), so
        advantage = kl_coef * (teacher - student), matching on-policy distillation.
        ...
        # Same sign as OnPolicyDistill: advantage = kl_coef * (teacher - student) when lambda_coef=1

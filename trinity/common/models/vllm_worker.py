...
class WorkerExtension:
    ...
    def update_weight(self, source: str = "student"):
        """Broadcast weight to all vllm workers from source rank 0 (actor model)"""
        if not hasattr(self, "_weight_update_rank"):
            raise RuntimeError(
                "init_process_group must be called before update_weight. "
                "Ensure setup_weight_sync_group (or setup_model_level_weight_sync_group) "
                "is invoked for all models including auxiliary models (e.g. teacher)."
            )
        if self._weight_update_rank == 0:
            state_dict, model_version = ray.get(
                self.synchronizer.get_model_state_dict.remote(source)
            )
        ...
        # For NCCL student sync, metadata is provided during init_process_group.
        # Fall back to synchronizer metadata for other sources or legacy paths.
        state_dict_meta = self._state_dict_meta if source == "student" else None
        if state_dict_meta is None:
            state_dict_meta = ray.get(self.synchronizer.get_state_dict_meta.remote(source))
        if state_dict_meta is None:
            raise RuntimeError(f"No state dict metadata found for source `{source}`.")
        for name, dtype_str, shape in state_dict_meta:
            if self._weight_update_rank == 0:

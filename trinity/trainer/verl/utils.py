...
def to_data_proto(
    ...
    # For hint OPD: teacher_logprobs_valid_mask when all experiences have it
    if all(
        getattr(exp, "teacher_logprobs_valid_mask", None) is not None
        for exp in experiences
    ):
        batch_dict["teacher_logprobs_valid_mask"] = gather_response_attrs(
            experiences, "teacher_logprobs_valid_mask", max_response_length, pad_value=False
        )
